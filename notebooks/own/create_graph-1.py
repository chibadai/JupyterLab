class create_source_to_graph(object):
    def __init__(
        ...
        self.module = importlib.import_module(import_module_name)
        if module_function is not None:
            self.source_lines = inspect.getsourcelines(getattr(self.module, module_function))
        else:
            self.source_lines = inspect.getsourcelines(self.module)
        ...
        self.recreate_source_lines()

        self.graph_source_list = []
        self.create_source_line_to_json()
    ...
    def get_source_lines(self):
        ...
        for idx in range(0, len(self.sources)):
            ...
            if def_line == 0 and commentout_flg == 0:
                ...
                print(indent_num, colon_flg, indent_before_after_num, len_split_before_line, len_split_after_line, output_line)
    ...
    def recreate_source_lines(self):
        z_fill_num = 2
        start_num = None
        source_idx = 0
        pop_counter = 0
        ...
        for idx in range(0, len_result_source_lines):
            ...
            if len_split_before_line == len_split_after_line:
                ...
                for line_idx in range(idx + 1, len(result_source_lines)):
                    ...
                    self.recreate_result_source_lines[-1][-1] += t_output_line.replace(replace_line_indent, '')
    ...
    def create_source_line_to_json(self):
        space_str = ' '
        before_brackets = '{'
        after_brackets = '}'
        subgraph_counter = 0
        ...
        self.graph_source_list.append(before_brackets + double_quotation + 'g' + double_quotation + colon + before_brackets)
        subgraph_counter += 1
        ...
        for idx in range(0, len(self.recreate_result_source_lines)):
            ...
            if before_indent_num is None or before_indent_num == indent_num:
                self.graph_source_list.append('\t'*subgraph_counter + node_name + space_str + before_brackets + double_quotation + 'label' + double_quotation + colon + double_quotation + line + double_quotation + after_brackets + commna)
    ...
    def create_edges_list(self):
        ...
        for idx in range(0, self.indent_max):
            # target_indent_num = 0
            print('create_edges_list idx', idx)
